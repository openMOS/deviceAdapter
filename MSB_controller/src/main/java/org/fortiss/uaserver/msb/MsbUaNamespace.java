...
import org.fortiss.uaserver.msb.method.ChangeStateMethod;
...
public class MsbUaNamespace implements Namespace {
	...
	public MsbUaNamespace(OpcUaServer server, UShort namespaceIndex) {
		this.server = server;
		...
		testFolder = new UaFolderNode(nodeManager, testFolderNodeId, new QualifiedName(namespaceIndex, "Test"),
		...
		try {
			server.getUaNamespace().addReference(Identifiers.ObjectsFolder, Identifiers.Organizes, true,
					testFolderNodeId.expanded(), NodeClass.Object);
		} catch (UaException e) {
	...
	private void addMethodNodes() {
		UaMethodNode methodNode = new UaMethodNode(server.getNodeMap(), new NodeId(getNamespaceIndex(), "changeState"),
				new QualifiedName(getNamespaceIndex(), "changeState"), new LocalizedText(null, "changeState"),
				LocalizedText.english("change state method to be called from DA"), UInteger.valueOf(0),
				UInteger.valueOf(0), true, true);

		ChangeStateMethod method = new ChangeStateMethod();

		AnnotationBasedInvocationHandler invocationHandler;
		try {
			invocationHandler = AnnotationBasedInvocationHandler.fromAnnotatedObject(nodeManager, method);
		...
		server.getNodeMap().put(new NodeId(getNamespaceIndex(), "changeState"), methodNode);

		server.getNodeMap().addReference(new Reference(Identifiers.ObjectsFolder, Identifiers.HasComponent,
	...
	public void write(WriteContext context, List<WriteValue> writeValues) {
		...
		for (WriteValue writeValue : writeValues) {
			try {
				...
				node.writeAttribute(new AttributeContext(server), writeValue.getAttributeId(), writeValue.getValue(),
	...
	private static final Object[][] STATIC_SCALAR_NODES = new Object[][] {
			{ "Bool", Identifiers.Boolean, new Variant(false) }, { "Byte", Identifiers.Byte, new Variant(ubyte(0x00)) },
			...
			{ "Duration", Identifiers.Duration, new Variant(1.0) }, { "Float", Identifiers.Float, new Variant(3.14f) },
			...
			{ "TestHappy", Identifiers.Int32, new Variant(32) }, };
	...
	private UaObjectNode addFoldersToRoot(UaNode root, String path) {
		...
		if (!nodeManager.containsKey(firstNode.getNodeId())) {
			...
			nodeManager.get(root.getNodeId()).addReference(new Reference(root.getNodeId(), Identifiers.Organizes,
    ...
    private void addHeartbeatNodes() {
        NodeId configNodeId = new NodeId(namespaceIndex, "HeartbeatConfig");

        UaObjectNode configNode = new UaObjectNode(
            nodeManager,
            configNodeId,
            new QualifiedName(namespaceIndex, "HeartbeatConfig"),
            LocalizedText.english("HeartbeatConfig")
        );

        nodeManager.put(configNodeId, configNode);
        ...
        try {
            server.getUaNamespace().addReference(
                ...
                true,
                configNodeId.expanded(),
                NodeClass.Object
            );
        } catch (UaException e) {
            logger.error("Error adding reference to HeartbeatConfig Object.", e);
        }

        NodeId portNodeId = new NodeId(namespaceIndex, "HeartbeatConfig/Port");

        UaVariableNode portNode = new UaVariableNode.UaVariableNodeBuilder(nodeManager)
            .setNodeId(portNodeId)
            ...
            .setBrowseName(new QualifiedName(namespaceIndex, "Port"))
            .setDisplayName(LocalizedText.english("Port"))
            .setDataType(Identifiers.UInt16)
            .setTypeDefinition(Identifiers.BaseDataVariableType)
            .setDescription(LocalizedText.english("Port where the heartbeat server is listening"))
            .build();
        portNode.setValue(new DataValue(new Variant(14840)));

        nodeManager.addNode(portNode);
        configNode.addProperty(portNode);

        NodeId timeoutNodeId = new NodeId(namespaceIndex, "HeartbeatConfig/Timeout");

        UaVariableNode timeoutNode = new UaVariableNode.UaVariableNodeBuilder(nodeManager)
            .setNodeId(timeoutNodeId)
            .setAccessLevel(ubyte(AccessLevel.getMask(AccessLevel.READ_ONLY)))
            .setUserAccessLevel(ubyte(AccessLevel.getMask(AccessLevel.READ_ONLY)))
            .setBrowseName(new QualifiedName(namespaceIndex, "Timeout"))
            .setDisplayName(LocalizedText.english("Timeout"))
            .setDataType(Identifiers.UInt32)
            .setTypeDefinition(Identifiers.BaseDataVariableType)
            .setDescription(LocalizedText.english("Interval in ms which the clients should use for the heartbeat"))
            .build();
        timeoutNode.setValue(new DataValue(new Variant(2000)));

        nodeManager.addNode(timeoutNode);
        configNode.addProperty(timeoutNode);
    }
	...
	private LinkedList<UaObjectNode> processPathElements(List<String> elements, List<String> path,
		...
		if (elements.size() == 1) {
			...
			UaObjectNode node = UaObjectNode.builder(nodeManager).setNodeId(new NodeId(namespaceIndex, prefix + name))
					.setBrowseName(new QualifiedName(namespaceIndex, name)).setDisplayName(LocalizedText.english(name))
