...
import org.fortiss.uaserver.device.lowlevel.subscriptions.SubscriptionGluing;
import org.fortiss.uaserver.device.lowlevel.subscriptions.SubscriptionLboro;

public class Glueing extends LboroSkillMethod {

    public Glueing(String stationName) {
        super();
        skillSubscription = new SubscriptionGluing(stationName);
    ...
    public void setMethod(MsbGenericComponent msbComponent, String deviceAdapterId, String recipeId,
        ...
        if (!inputArguments.isEmpty()) {
            this.programId = inputArguments.get("GlueingProfile/SetValue");
            this.speed = inputArguments.get("ProcessSpeed/SetValue");
        }
    }
